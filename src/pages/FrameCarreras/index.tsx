...
export default function CareersPage() {
  ...
  const [carreraSeleccionada, setCarreraSeleccionada] = useState<Career | null>(
    null,
  );
    ...
    const fetchRecommendations = async () => {
      try {
        ...
        const ids = new Set<string>(
          res.recommendations.map((r: any) => r.career_id).filter(Boolean),
        );
      ...
      <div className="flex flex-col self-stretch max-w-[1152px] mb-8 mx-auto gap-8 mt-10 px-4">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6">
          <div className="flex flex-col items-start gap-4">
            <span className="text-[#0D0D1B] text-5xl font-black">
              Explora Carreras Universitarias
            </span>
            <span className="text-[#4C4C9A] text-lg max-w-[651px] font-normal">
              Utiliza nuestra inteligencia artificial para encontrar la carrera
              que mejor se adapta a tus pasiones y metas profesionales.
            </span>
          </div>
        ...
        <div className="flex items-center gap-4 overflow-x-auto pb-2">
          ...
          >
            <img
              src={
                "https://storage.googleapis.com/tagjs-prod.appspot.com/v1/y0WLx2RbqX/16w66z7y_expires_30_days.png"
              }
              className="w-3.5 h-5 object-contain"
              style={{
                filter:
                  filtroActivo === "IA" ? "brightness(0) invert(1)" : "none",
              }}
              alt="IA Icon"
            />
          ...
          <button
            ...
            onClick={() => {
              setFiltroActivo("Ingeniería");
              setPage(1);
            }}
          >
            <img
              src={
                "https://storage.googleapis.com/tagjs-prod.appspot.com/v1/y0WLx2RbqX/il8iu3gh_expires_30_days.png"
              }
              className={`w-3.5 h-5 object-contain ${filtroActivo === "Ingeniería" ? "brightness-0 invert" : ""}`}
              alt="Ingeniería Icon"
            />
          ...
          <button
            ...
            onClick={() => {
              setFiltroActivo("Salud");
              setPage(1);
            }}
          >
            <img
              src={
                "https://storage.googleapis.com/tagjs-prod.appspot.com/v1/y0WLx2RbqX/3ns51jb1_expires_30_days.png"
              }
              className={`w-3.5 h-5 object-contain ${filtroActivo === "Salud" ? "brightness-0 invert" : ""}`}
              alt="Salud Icon"
            />
      ...
      <div className="flex flex-col self-stretch max-w-[1152px] mx-auto gap-6 mb-20 px-4 min-h-[400px]">
        ...
        {carrerasParaMostrar.length > 0 ? (
          carrerasParaMostrar.map((carrera) => (
            <div
              key={carrera.id}
              className="flex flex-col md:flex-row items-center bg-[#F6F9FA] p-8 gap-8 rounded-[48px] hover:shadow-lg transition-shadow duration-300"
            >
              <div className="w-32 h-32 rounded-[32px] bg-white flex items-center justify-center shadow-sm shrink-0">
                <img
                  src={
                    carrera.imagen || "https://placehold.co/100x100?text=Uni"
                  }
              ...
              <div className="flex flex-1 flex-col gap-2 w-full">
                ...
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2">
                    <span className="bg-blue-100 text-blue-800 text-xs px-2 py-0.5 rounded-md font-bold uppercase">
                      {carrera.area}
                    </span>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-gray-500 mt-2 font-normal">
              Prueba buscando en "Todas" o revisa la ortografía.
            </p>
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-[32px] w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl animate-fade-in-up">
            <div className="relative h-40 bg-[#1313EC] rounded-t-[32px] flex items-center justify-center">
              ...
              <div className="w-24 h-24 bg-white rounded-3xl flex items-center justify-center border-4 border-white shadow-lg absolute -bottom-12">
                <img
                  src={
                    carreraSeleccionada.imagen ||
                    "https://placehold.co/100x100?text=Uni"
                  }
                  className="w-14 h-14 object-contain"
                  alt={carreraSeleccionada.nombre}
                />
              </div>
            </div>
            ...
            <div className="pt-16 pb-8 px-8 flex flex-col gap-6 text-center">
              ...
              <div className="grid grid-cols-2 gap-4 text-left">
                <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                  ...
                  <p className="text-[#0D0D1B] font-bold mt-1">
                    {carreraSeleccionada.salarioPromedio}
                  </p>
                </div>
                <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                  <span className="text-xs text-gray-500 uppercase font-bold">
                  ...
                  <p className="text-[#0D0D1B] font-bold mt-1">
                    {carreraSeleccionada.duracion}
                  </p>
                </div>
              </div>

              {filtroActivo === "IA" &&
                (carreraSeleccionada.matchIA ?? 0) > 0 && (
              ...
              <div className="text-left">
                <h4 className="text-[#0D0D1B] font-bold text-sm mb-3">
                  Disponible en estas Universidades:
                </h4>
                <div className="flex flex-wrap gap-2">
                  {carreraSeleccionada.universidades?.length > 0 ? (
                    carreraSeleccionada.universidades.map(
                      (uni: string, idx: number) => (
                        <span
                          key={idx}
                          className="bg-white border border-gray-300 px-3 py-1.5 rounded-full text-xs text-gray-700 font-medium hover:border-[#1313EC] hover:text-[#1313EC] cursor-pointer transition-colors"
                          onClick={() => navigate("/universidades")}
                        >
                          {uni}
                        </span>
